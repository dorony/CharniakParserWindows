...
ewDciTokStrm( const ECString& name )
  ...
  sentenceName(L""),
  istr_(name.c_str()),
  savedWrd_( L"" ),             // holds not-yet-processed parts of current Wrd
  nextWrd_( L"" ),              // holds "on-deck" Wrd
...
{      
  ECString str;
  if(useCin && !cin) return L"";
  if(!useCin && !istr_) return L"";
  if(useCin)   cin >> str;
  else istr_ >> str;  
  return str;
}
...
/*** Execution reaches this point iff. a new Wrd has just "come on deck." ***/
    ...
    if( savedWrd_ == L"<COMMENT>" )              // Comments occur only as the
    {                                           //  last Wrds in a <s>...</s>
	nextWrd_ = flush_to_sentence();         //  line.  Discard the comment
	savedWrd_ = L"";                         //  itself and the "Paragraph-
	return L"</s>";                          //  ing Error" (unbracketed
    }                                           //  text) that follows.

    while( savedWrd_ == L"<s>"               // Starting in 1989, lines brack-
           &&  nextWrd_ == L"@" )            //  eted as sentences but with an
    {                                       //   @  following the  <s>  hold
        //wcerr << "before @ flush" << endl;				      
	savedWrd_ = flush_to_sentence();    //  data arranged as charts or
        //wcerr << "aft @ flush" << endl;				      
	if(useCin)
	  {
	    if( !cin )                        //  tables.  It is non-text-like
	      nextWrd_ = L"";                  //  and should be discarded.
	    else cin >> nextWrd_;
	  }
	else
	  {
    ...
    if( ellipFlag && retWrd != L"." ) { ellipFlag = 0; }             // update
    if( retWrd == L"." ) { ellipFlag++; }                            // flags
    if( nextWrd_.length() && nextWrd_[0] == '('                      // before
	&& ( retWrd == L"!" || retWrd == L"?"                        // return
	     || retWrd == L"." && ellipFlag != 3 ))                  //    .
    { parenFlag = 1; }                                               //    .
                                                                     //    .
    if( retWrd == L"(" )                                             //    .
	...
	{ parenFlag = 0; }                                           //    .
                                                                     //    .
    if( retWrd == L")" || retWrd == L"</s>" || retWrd == L"</S>" ) { parenFlag = 0; }     //.....

    if(retWrd == L"(") retWrd = L"-LRB-";
    if(retWrd == L")") retWrd = L"-RRB-";
    if(retWrd == L"{") retWrd = L"-LCB-";
    if(retWrd == L"}") retWrd = L"-RCB-";
    if(retWrd == L"[") retWrd = L"-LSB-";
    if(retWrd == L"]") retWrd = L"-RSB-";
    // a hack so that '' comes out as closed quote;
    if(savedWrd_ == L"\'\'") savedWrd_ = L"\"";
    return retWrd;
}
...
/** In the raw text, the closing dot of a "stateLike" abbreviation may serve
								...
								// page FIVE
	{
		...
		savedWrd_ = L".";	     //  pected trailing characters,
	...
	{                             // trailing-or-internal-apostrophe cases
		...
		if( length > 4 )
		{
		    if( seq.substr( 0,5 ) == L"can't" || seq.substr( 0,5 ) == L"Can't" )
		    {   savedWrd_ = seq.substr( 2,length-2 );
		        ECString retv =  seq.substr( 0,2 );
			retv += L"n";
                	return retv;
		    }
		    if( seq.substr( 0,5 ) == L"won't" || seq.substr( 0,5 ) == L"Won't" )
		    {	savedWrd_ = seq.substr( 2,length-2 );
		        ECString retv =  seq.substr( 0,1 );
			retv += L"ill";
                	return retv;
		    }
		    if( seq.substr( 0,5 ) == L"ain't" || seq.substr( 0,5 ) == L"Ain't" )
		    {	savedWrd_ = seq.substr( 2,length-2 );
                	return L"IS";       // caps as warning: "am"|"are"|"is"
		    }
		}
                                                                 // page SIX
	    ...
	    if( length > 4 &&
		  ( seq.substr( 0,5 ) == L"goin'" || seq.substr( 0,5 ) == L"doin'" )
	       || length > 6 &&
		  ( seq.substr( 0,7 ) == L"lookin'" || seq.substr( 0,7 ) == L"fishin'" ) )
	    {
		savedWrd_ = seq.substr( puncIndex+1,length-puncIndex-1 );
		return seq.substr( 0, puncIndex+1 );
	    }
	    ...
	    if( length > 2 &&
		(  seq.substr(0,3) == L"A's"   //eg  Oakland A's    Type A's die young
		|| seq.substr(0,3) == L"O's"    // the Orioles are a minor league team
		|| seq.substr(0,3) == L"R's" ))   //eg    the three R's
	    {
...
**   happen to mean close-quote, and that's how this rule treats them.    ***/
	...
	if( has_alnum( seq,0 )
	    || seq == L"\"." && nextWrd_ == L"." )   //eg  he said, ". . . Yes!"
	{   return L"``";  }
	else { return L"''"; }
    }
	...
	if( seq == L"'cause" )               //eg   ". . . just 'cause I care!"
	{
	    savedWrd_ = L"";
	    return seq;
	}
    ...
    if( seq[puncIndex] != '(' && seq[puncIndex] != ')'
	&&seq[puncIndex] == seq[ puncIndex+1 ] )    // treat string of identical
    {                                             //  puncChars as a Wrd except().
	int lastSame = 1;
...
is_stateLike( const ECString &str )   // that shouldn't be split off from it.
{
    ...
    ||	str == L"Ala."  
    ||	str == L"Ca."	|| str == L"Calif."	|| str == L"Conn." 
    ||	str == L"Fla."	|| str == L"Ga."		|| str == L"Ill."
    ||	str == L"Ky."	|| str == L"La."		
    || str == L"Mass."	|| str == L"Me."
    ||	str == L"Md."	|| str == L"Mich."	|| str == L"Mo."
    ||	str == L"Pa."	|| str == L"Va."		|| str == L"Vt."
    ||	str == L"Wash."	|| str == L"Wyo."	|| str == L"W.Va."
    ||	str == L"Co."	|| str == L"co."		|| str == L"Ltd."
    ||	str == L"Cos."	|| str == L"cos."	|| str == L"Corp."
    ||	str == L"Inc."	|| str == L"INC."	|| str == L"CORP."
    ||	str == L"Jr."	|| str == L"Sr." 	|| str == L"Blvd."
    ||	str == L"St."	|| str == L"Ave."
    );
}
