...
ECString Term::Language = L"En";

bool
Term::
isS() const 
{
  if(Term::Language == L"Ch") return name_== L"IP";
  else return name_ == L"S";
}
...
init(ECString & prefix)
{
  ...
  fileName += L"terms.txt";
  ifstream           stream(fileName.c_str(), ios::in);
  if (!stream)
    {
      wcerr << "Can't open terms file " << fileName << endl;;
      return;
    }
  ...
  while (stream >> termName)
    {
      ...
      if(termName == L"STOP") Term::stopTerm = nextTerm;
      else if(termName == L"G4") Term::startTerm = nextTerm;
      else if(termName == L"S1") Term::rootTerm = nextTerm;
      array_[n] = nextTerm;
      if(!ind && !seenNTs)
	{
	  assert(n > 0);
