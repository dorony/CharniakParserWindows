...
follow(int val, int auxCnt)
{
  ...
  if(!auxNd)
    {
      return NULL;
      //wcerr << "No auxnd " << *this << ", " << val << ", " << auxCnt << endl;
      //assert(auxNd);
    }
...
readOneLevel0(istream& is, int c)
{
  ...
  is >> nextIndStr;
  if(!is) return -1;
  if(nextIndStr == L"Selected") return -1;
