...
{
  //int hsz = heap.size();
  //wcerr << "Need " << n << "th variation out of " << num()
  ...
  for(int i = 0 ; i < val->len() ; i++)
    {
      ...
      if(nv)
	{
	  //wcerr << "Got the possible variation " 
	  //   << *nv  << " "<< heap.size()<< " "
	  //   << i << " " << n << " " << num() << endl;
	  //assert(ptst(nv));
	  heap.push(nv);
  ...
  Val* ans = heap.pop();
  //assert(ans->check());
  //wcerr << "The desired variation is " << *ans << " " << num_ << " "
  //   << hsz << " " << heap.size() << endl;
  /*
  if(!ans->edge())
    {
      ...
      else
	{
	  ...
	  wcerr << "Subedge = " << *subans->edge() << endl;
	}
    }
  */
  nbest.push_back(ans);
  ...
  if(status == EXTRAVAL)
    {
      ...
      else return bsts_.front()->nth(0)->trm();
    }
  else return trm_;
}
...
newIth(int ith, Val* oval, bool& stop)
{
  ...
  if(ithc > 0) stop = true;
  short nxtI = ithc +1;;
  //wcerr<< "Wnt " << nxtI << "th var on pos " << ith << " of " << *oval<< endl;
  ...
  //wcerr << "ith is " << ith << " " << ans->vec().size() << " "
  //   << ans->vec()[ith] << " " << frac << endl;
  //wcerr << "The var is " << *ans << endl;
  //assert(ans->check());
  return ans;
}
