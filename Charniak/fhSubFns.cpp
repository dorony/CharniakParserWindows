...
{
  if(parw == L"am" || parw == L"was" || parw == L"is" || parw == L"be" || parw == L"been"
     || parw == L"are" || parw == L"were" || parw == L"being")
    return 1;
  else return 0;
}
...
fh_term_before(FullHist* fh)
{
  ...
  error(L"Should never get here fh_term_b");
  return -1;
}
...
fh_term_after(FullHist* fh)
{
  ...
  error(L"Should never get here fh_term_after");
  return -1;
}
...
is_effEnd(FullHist* tree, FullHist* child)
{
  ...
  for( ; ; loc++)
	...
	return is_effEnd(tree->back,tree);
      ...
      if(ntrmNm == L"''") return 1;
      return 0;
    }
  error(L"should not get here");
  return 0;
}
...
fh_mE(FullHist* treeh)
{
  ...
  int e = is_effEnd(treeh->back, treeh);
  //if(treeh->e) wcerr << "Me " << e << " " << treeh->term << endl;
  return m+(e*150);
}
...
{
  error(L"fh_Bl should never be called");
  return -1;
}
...
{
  error(L"fh_Br should never be called");
  return -1;
}
...
int
tree_watpos(int pos,FullHist* treeh)
{
  if(pos < 0) return nullWordInt;
  assert(treeh->cb);
  int ans = treeh->cb->sentence_[pos].toInt();
  //assert(ans >= 0);
  return ans;
}
...
{
  int zpos = treeh->pos; 
  return tree_watpos(zpos-1,treeh);
}
...
{
  int zpos = treeh->pos;
  return tree_watpos(zpos-2,treeh);
}
...
addSubFeatureFns()
{
  ...
  ECString temp(Bchart::HEADWORD_S1);
  nullWordInt = Bchart::wordMap[temp];
}
