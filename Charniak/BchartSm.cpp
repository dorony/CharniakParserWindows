  ...
  for(int f = 0 ; f < 20 ; f++)
    {
      ...
      if(!pUstream)
	{
	  wcerr << "Could not find " << pUString << endl;
	  assert(pUstream);
	}
      for( i = 0 ; i <  nm ; i++ )
	{
...
readTermProbs(ECString& path)
{
  ...
  pTstring += L"endings.txt";
  ECString ppTstring(path);
  ppTstring += L"nttCounts.txt";
...
initDenom()
{
  ...
  for(i = 0 ; i < wrd_count_ ; i++)
	...
	{	
	  ...
	  wpli++;
	  float prb = *wpli;
	  if(prb == 0) wcerr << "Zero prob from wordPlist, "
	  ...
	  if(printDebug(1000))
	    wcerr << "initD " << i << "\t" << trmInt << "\t" << pw0nt << endl;
	  pw0n += pw0nt;
	}

      if(pw0n == 0)
	{
	  wcerr << "Zero at pos " << i << " word = "
	   << sentence_[i] << endl;
	  ...
	  for( ; wpli != wpl.end() ; wpli++)
	    {
	      ...
	      wcerr << "Assigning " << trmInt << " prob = " << pw0nt << endl;
	      parray[trmInt][1] = pw0nt;
	      pw0n += pw0nt;
	    }
	}
  ...
  if(printDebug(1000))
    wcerr << "initD " << wrd_count_ << "\tSTOP("<<eosInt<<")\t" << ans << endl;
...
wtoInt(ECString& w)
{
  ...
  wmi = newWordMap[thrdid].find(w);
  if(wmi != newWordMap[thrdid].end()) return (*wmi).second;
  lastWord[thrdid]++;
...
intToW(int n)
{
  ...
  else return newWords[thrdid][n-lastKnownWord-1];
}
...
wordPlist(Wrd* word, int word_num)
{
  ...
  if( wint <= lastKnownWord )
      ...
      for( i = 0 ; i <= Term::lastNTInt() ; i ++ )
	{
	  ...
	  ans.push_back(prob);
	  if(printDebug(7777)) wcerr << "\t" << i << "\t" << prob << endl;
	}
      if(!ans.empty())
	{
	  return ans;
  ...
  for(int i = 0 ; i <= Term::lastTagInt() ; i++)
    {
      ...
      if(prob == 0) continue;
      assert(prob > 0);
      if(printDebug(7777)) wcerr << "Uk\t" << i << "\t" << prob << endl;
      ans.push_back((float)i);
      ans.push_back(prob);
    }
  return ans;
...
pCapgt(const Wrd* shU, int t)
{
  ...
  if(Term::Language == L"Ch") return 1.0;
  int word_num = shU->loc();
  const ECString& lex0 = sentence_[0].lexeme();
  if(word_num == 0) return 1;
  else if(word_num == 1 &&
	  (lex0 == L"``" || lex0 == L"-LCB-" || lex0 == L"-LRB-"))
    return 1;
  //wcerr << "pCapgt = " << pcap << endl;
...
{
  //wcerr << "Unknown word: " << shU << " for tag: " << t << endl; 
  double ans = pHugt(t);
  //wcerr << "pHugt = " << ans << endl;
  assert(ans >= 0);
  if(ans == 0) return 0;
  double phyp = 1;
  if(Term::Language != L"Ch") phyp = pHypgt(shU->lexeme(),t);
  ans *= phyp;
  //wcerr << "pHypgt = " << phyp << endl;
  double phcp = 1;
  if(Term::Language != L"Ch") phcp = pCapgt(shU,t);
  ans *= phcp;
  ans *= .0001;
  assert(ans >= 0);
...
pHypgt(const ECString& shU, int t)
{
  ...
  bool hyp = false;
  size_t found = shU.find('-');
 /* Char* hyppos =  strpbrk(shU.c_str(), "-");*/
  if(found!=string::npos) hyp = true;
...
computeMerit(Edge* edge, int whichDist)
{
  ...
  if(printDebug(150)) wcerr << "cM" << whichDist << " = " << ans << endl;
  if(smallCorpus && ans == 0) ans = .000001;
  return ans;
}
