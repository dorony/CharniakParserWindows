...
{
  if(conditioned == L"h") whichInt = HCALC;
  else if(conditioned == L"u") whichInt = UCALC;
  else if(conditioned == L"r") whichInt = RCALC;
  else if(conditioned == L"ru") whichInt = RUCALC;
  else if(conditioned == L"rm") whichInt = RMCALC;
  else if(conditioned == L"tt") whichInt = TTCALC;
  else if(conditioned == L"l") whichInt = LCALC;
  else if(conditioned == L"lm") whichInt = LMCALC;
  else if(conditioned == L"s") whichInt = SCALC;
  else if(conditioned == L"t") whichInt = TCALC;
  else if(conditioned == L"ww") whichInt = WWCALC;
  else
    {
      assert(conditioned == L"m");
      whichInt = MCALC;
    }
}
...
{
  ECString npath(L"/ltmp/discrim/FOLDS/");
  int b,f;
  int tot = Feature::total[which];
  for(int i = 0 ; i < 20 ; i++)
    {
      ECString ftstr(npath);
      ftstr += intToString(i) + L"/";
      ftstr += tmp;
      ftstr += L".lambdas";
      ...
      for(b = 1; b < 15 ; b++)
	{
	  ...
	  if(!fts)
	    {
	      wcerr << "Trouble reading lambs for " << which << " in " << ftstr
		   << endl;
	      assert(fts);
	    }
	  fts >> bb ;
