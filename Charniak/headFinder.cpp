...
readHeadInfoEn(ECString& path)
{
  ...
  while(headStrm)
      ...
      if(next == L"2")
	{
      ...
      headStrm >> next2;
      //wcerr << "NN " << next << " " << next2 << endl;
      if(!headStrm) error(L"Bad format for headInfo.txt");
      next += next2;
...
headPriority(ECString lhsString, ECString rhsString, int ansPriority) 
{
  ...
  both += rhsString;
  if(lhsString == L"PP" && ansPriority == 1) return 10;//make fst IN head of PP
  ...
  else if(!rhsTerm->terminal_p() && rhsTerm->name() != L"PP")
    return 5;
  ...
  for( ; subTreeIter != tree->subTrees().end() ; subTreeIter++ )
    {
      ...
      //wcerr << "Npri " << nextPriority << lhsString << " " << rhsString
      //   << endl;
...
{
  if(Term::Language == L"Ch") return headPosFromTreeCh(tree);
...
{
  if(Term::Language == L"Ch") return readHeadInfoCh(path);
