  ...
  for(int i = 0 ; i < NTH ; i++)
    {
      ...
      //wcerr << "CBAT " << p1 << " " << ats.trees[i].trm << endl;
    }
}
  ...
  for(int i = 0 ; i < lim ; i++)
    {
      ...
      atp.first = -1;
      //wcerr << "CBSB " << i << " " << atp.first << " " << atp.second.trm << endl;
    }
}
  ...
  for(int i = 0 ; i < lim ; i++)
    {
      ...
      assert(ats.trees[i].trm >= 0);
      atp.first = -1;
      //wcerr << "CBSBGH " << i << " " << atp.first << " " << atp.second.trm << endl;
    }
}
...
CombineBestsT(const Term* tm, double prb) : rprob(prb), trm(tm), whichIsCur(false)
{
  ...
  at.trm = trm->toInt();
  curAtq().push(prb, at);
  //wcerr << "CBT " << this << " " << curAtq().size << endl;
}
...
{
  switchQueues(); //???;
  //wcerr << "ET0 " << this << " " << pastAtq().size << " " << curAtq().size << endl;
  for(int i = 0 ; i < NTH ; i++)
    {
      ...
      for(int j = 0 ; j < pastAtq().size ; j++)
	{
	  ...
	  //wcerr << "CBET " << i << " " << p2 << " " << tree2.trm << endl;
	}
      if(!addedNew) break;
    }
}
