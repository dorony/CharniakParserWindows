...
LeftRightGotIter* globalGi[MAXNUMTHREADS];
...
init(ECString path)
{
  ...
  ECString tmpA[MAXNUMCALCS] = {L"r",L"h",L"u",L"m",L"l",L"lm",L"ru",L"rm",L"tt",
				L"s",L"t",L"ww",L"dummy",L"dummy",L"dummy"};

  for(int which = 0 ; which < Feature::numCalcs ; which++)
    {
      ECString tmp = tmpA[which];
      Feature::init(path, tmp); 
      if(tmp == L"s" || tmp == L"t") continue;
      ECString ftstr(path);
      ftstr += tmp;
      ftstr += L".g";
      ifstream fts(ftstr.c_str());
      if(!fts) wcerr << "could not find " << ftstr << endl;
      assert(fts);
      FeatureTree* ft = new FeatureTree(fts); //puts it in root;
      if(tmp == L"ww") continue;
...
bestParse(Item* itm, FullHist* h, Val* cval, Val* gcval, int cdir)
{
  ...
  if(bst.explored())
	...
	{
	  prDp();
	  wcerr << "already known bestParse(" << *itm << ", ...) has p = "
	       << bst.prob() << endl;
	}
      return bst;
    }
  ...
  for( ; pi != pm.end() ; pi++ )
	...
	{
	  prDp();
	  wcerr << "consider Pos(" << *itm << ") = " << posInt << endl;
	}
      ...
      for( ;hi != hm.end();hi++)
	{
	  ...
	  if(printDebug() > 16)
	    {
	      prDp();
	      wcerr << "consider head(" << *itm << ") = " << subh << endl;
	    }
	  float hprob = 0;
	  if(wrdInt >= 0 && wrdInt <= lastKnownWord)
	    {
	      ...
	      if(hprob < 0)
		{
		  wcerr << posInt << " " << pHugt(posInt) <<" "<<hprob2 << endl;
		  assert(hprob >=0);
		}
	    }
	  ...
	  if(printDebug() > 16)
	    {
	      prDp();
	      wcerr << "p(hd) = "<< hprob << endl;
	    }
	  float hhprob = (hposprob * hprob);
	  if(hhprob < 0)
	    {
	      wcerr << hposprob << " " << hprob << endl;
	      assert(hhprob >= 0);
	    }
	  h->hd = &subhw;
	  Bst&  
  ...
  if(printDebug() > 10)
    {
      prDp();
      wcerr << "Bestp for " << *itm << " = " << bst.prob() <<endl;
    }
  return bst;
}
		   ...
		   FullHist* h, ItmGHeadInfo& ighInfo, Val* cval, Val* gcval)
{
  ...
  if(printDebug() > 10)
    {
      prDp();
      wcerr << "bestParseGivenHead(" << posInt << ", " << wd
	   << ", " << *itm  << ")" << endl;
    }
  double bestP = 0;
  double sumP = 0;
  ...
  if(printDebug() > 10)
    {
      prDp();
      wcerr << "Bestpgh for "<<*itm << ", " << wd << " = " << bst.prob()<< endl;
    }
  return bst;
}
...
meProb(int cVal, FullHist* h, int whichInt)
{
  if(printDebug() > 68)
    {
      prDp();
      wcerr << "meP" << whichInt << "(" << cVal << " | " << *h << ")" <<endl;
    }
  ...
  for(int i = 1 ; i <= Feature::total[whichInt] ; i++)
      ...
      if(i == 1)
	{
	  ...
	  if(!histPt)
	    {
	      wcerr << cVal << " " << whichInt << " " << nfeatV << " " << searchStartInd <<" " << feat->auxCnt << endl;
	      assert(histPt);
	    }
	  ...
	  if(!f)
		...
		{
		  prDp();
		  wcerr << "Zero p" << feat->name << " " << nfeatV << endl;
		}
	  ...
	  if(printDebug() > 68)
	    {
	      prDp();
	      wcerr << i << " " << nfeatV << " " << smoothedPs[1] << endl;
	    }
  ...
  if(printDebug() > 30)
    {
      prDp();
      wcerr<<"p"<<whichInt<< "(" << cVal << "|" << *h << ") = " << ans << endl;
    }
  return ans;
}
