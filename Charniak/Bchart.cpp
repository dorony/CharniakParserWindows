...
const Char * Bchart::HEADWORD_S1 = L"^^";
...
put_in_reg(Item * itm)
{
	...
	error( L"illegal indices in put_in_reg" );
...
{
    if(printDebug() > 250) wcerr << "add_reg_item " << *itm << endl;
    put_in_reg(itm);
  ...
  for(int right = 0 ; right < 2 ; right++)
    {
      ...
      //wcerr<< "Look for " << *itm << " " << pos << " " << right << endl;
...
{
  if(printDebug() > 140)
    wcerr << "add_starter_edges " << *itm << endl;
  ConstTerm* poslhs;
...
extend_rule(Edge* edge, Item * item, int right)
{
    ...
    if(printDebug() > 140)
      wcerr << "extend_rule " << *edge << " " << *item << endl;
    ...
    newEdge->setmerit(); 
    //wcerr << "DEM " << tmp << " " << newEdge->merit() << endl;
...
addFinishedEdge(Edge* newEdge)
{
  ...
  if(printDebug() > 250)
    wcerr << "addFinishedEdge " << *newEdge << endl;
...
in_chart(const Wrd* hd, const Term * trm, int start, int finish)
{
    ...
    if( finish <= 0 || start < 0 || finish - start - 1 < 0 )
      {
	wcerr << "For " << *trm << "(" << start << ", " << finish << ")"
	  << endl;
	error(L"bogus boundary params in in_chart" );
      }
...
{
  //wcerr << "rpEdge " << *edge << endl;
  double oldEdgeP = edge->prob();
  //if(oldEdgeP == 0) wcerr << "Zprob " << *edge << endl;
...
redoP(Item *item, double probDiff)
{
  ...
  //wcerr << "P( " << *item << " ) goes from  " << oldItemP
    //<< " -> " << item->prob() << endl;
  item->storeP() = 0.0;
  if( oldItemP == 0.0 )
    {
...
setPosStarts()
{
  ...
  for(k = 0 ; k < ft->subtree.size() ; k++)
      ...
      for(l = 0 ; l < ft2->feats.size() ; l++)
	{
	  ...
	  //wcerr << "For posstart " << j << " headphrase = " << i << endl;
...
meFHProb(const Term* trm, FullHist& fh, int whichInt)
{
  ...
  if(printDebug() > 138)
    {
      wcerr << "meP " << *trm << " " << cVal << " " << whichInt << " ";
      if(edge) wcerr << *edge << endl;
      else wcerr << fh.preTerm  << endl;
    }
  ...
  for(int i = 1 ; i <= Feature::total[whichInt] ; i++)
      ...
      if(nfeatV < -1)
	{
	  if(printDebug() > 128)
	    {
	      wcerr<<"p"<<whichInt<< "(" << cVal << "|";
	      if(edge) wcerr << *edge;
	      else wcerr << fh.preTerm;
	      wcerr << ") = " << ans << endl;
	    }
	  return ans;
	}
